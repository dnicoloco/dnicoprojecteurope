#!/usr/bin/env python3
"""Upload tutor (channel 1) sentence-level utterances to Supabase via PostgREST.

Reads Deepgram JSON transcripts under /Users/dnico/Desktop/PREPLY/Student-1/lesson-*/*.json,
extracts tutor-channel (channel 1) sentences, resolves segment_id via a GET on lesson_segments,
...
NOTE: This script only INSERTs tutor utterances — it does NOT delete existing student rows.
"""
import json
import os
import pathlib
...
def extract_tutor_utterances(segment_map) -> list[dict]:
    out = []
    student_dir = BASE / "Student-1"
    if not student_dir.is_dir():
        print(f"Student-1 directory not found at {student_dir}", file=sys.stderr)
        sys.exit(1)
    sid = STUDENT_IDS["Student-1"]
    for lesson_dir in sorted(student_dir.iterdir()):
        if not lesson_dir.is_dir():
            continue
        key = ("Student-1", lesson_dir.name)
        if key not in LESSON_IDS:
            continue
        lid = LESSON_IDS[key]
        seen = set()
        ...
        for seg_idx, f in enumerate(files, start=1):
            ...
            # Channel 1 = tutor
            alt = d["results"]["channels"][1]["alternatives"][0]
...
def main():
    ...
    rows = extract_tutor_utterances(seg_map)
    print(f"extracted {len(rows)} tutor utterances")
    if not rows:
        print("no tutor utterances found — nothing to upload")
        return
    BATCH = 100
