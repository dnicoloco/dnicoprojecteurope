...
export type GravityRef = {
  ...
  stop: () => void;
  reset: () => void;
  /** Snap the physics body under this DOM element back to angle 0, killing spin. */
  resetElementAngle: (el: HTMLElement) => void;
};
...
const MatterBody = ({
  ...
  useEffect(() => {
    ...
    const idSnapshot = idRef.current;
    return () => context.unregisterElement(idSnapshot);
...
const Gravity = forwardRef<GravityRef, GravityProps>(
  (
      ...
      (id: string, element: HTMLElement, props: MatterBodyProps) => {
        ...
        if (props.bodyType === "circle") {
          ...
          body = Bodies.circle(x, y, radius, {
            ...(props.matterBodyOptions as Matter.IChamferableBodyDefinition),
            angle,
            render: {
    ...
    const initializeRenderer = useCallback(() => {
      ...
      // Sleeping was blocking MouseConstraint from waking bodies on grab, so
      // drags stopped working. Keep the world awake and use heavier air
      // friction on individual bodies to kill drift.
    ...
    const resetElementAngle = useCallback((el: HTMLElement) => {
      bodiesMap.current.forEach(({ element, body }) => {
        if (element === el) {
    ...
    useImperativeHandle(
      ...
      [startEngine, stopEngine, reset, resetElementAngle],
    );
